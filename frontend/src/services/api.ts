...
import { DUMMY_METRICS, DUMMY_SURVEYS, DUMMY_ANOMALIES, DUMMY_TRENDS } from '../data/dummyData';
...
export const getDashboardMetrics = async (portIdOrName?: string, signal?: AbortSignal): Promise<DashboardMetrics> => {
  try {
    ...
    console.warn("Backend unavailable, using dummy metrics.");
...
export const getSurveys = async (): Promise<Survey[]> => {
  try {
    ...
    console.warn("Backend unavailable, using dummy surveys.");
    return DUMMY_SURVEYS;
  }
};
...
export const getAnomalies = async (filters?: AnomalyFilters, portIdOrName?: string, signal?: AbortSignal): Promise<Anomaly[]> => {
  ...
  try {
    ...
    console.warn("Backend unavailable, using dummy anomalies.");
...
export const getAnomalyById = async (id: string, portIdOrName?: string, signal?: AbortSignal): Promise<Anomaly> => {
  ...
  try {
    ...
    console.warn("Backend unavailable, using dummy anomaly data.");
    const found = DUMMY_ANOMALIES.find(a => a.id === id);
    if (!found) throw new Error('Anomaly not found in dummy data');
    return found;
  }
};
...
export const getTemporalSeries = async (_anomalyId: string, portIdOrName?: string, signal?: AbortSignal): Promise<TemporalPoint[]> => {
  try {
    ...
    console.warn("Backend unavailable, using dummy temporal trends.");
    return DUMMY_TRENDS;
  }
};
...
export const getDashboardTrends = async (portIdOrName?: string, signal?: AbortSignal): Promise<TemporalPoint[]> => {
  try {
    ...
    console.warn("Backend unavailable, using dummy dashboard trends.");
    return DUMMY_TRENDS;
  }
};
