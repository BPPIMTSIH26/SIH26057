...
export default function SignupPage() {
  ...
  useEffect(() => {
    // Restore pending verification from sessionStorage (survives page refresh during cold start)
    const pending = sessionStorage.getItem('sagar_pending_verify');
    if (pending) {
      try {
        const { email: savedEmail } = JSON.parse(pending);
        if (savedEmail) {
          setEmail(savedEmail);
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 90-second timeout to handle Render free-tier cold start (~50s)
      ...
      if (data.code) {
        // Store code internally — show ONLY via resend button, not on screen
        setAutoCode(data.code);
      }
      // Persist email to sessionStorage so page refresh during cold start doesn't lose the screen
      sessionStorage.setItem('sagar_pending_verify', JSON.stringify({ email }));
      setShowVerification(true);
    } catch (err: any) {
      if (err.name === 'AbortError') {
        setError('The server is waking up (cold start). Please wait 60 seconds and try again, or refresh this page.');
      } else {
  ...
  const handleResend = async () => {
    setError('');
    setResendStatus('Requesting new code...');
    try {
      const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000/api';
      ...
      if (data.code) {
        setAutoCode(data.code);
        // Show code in status message — auto-clears after 30 seconds
        setResendStatus(`Your code: ${data.code}`);
        setTimeout(() => setResendStatus(''), 30000);
      } else {
      ...
      <div className="flex h-screen w-full bg-void text-text-primary overflow-hidden font-sans items-center justify-center relative">
        ...
        <div className="w-full max-w-md bg-glass backdrop-blur-3xl border border-glass-border p-8 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] relative z-10">
          ...
          <form onSubmit={handleVerifySubmit} className="space-y-4">
              ...
              <div className="mt-4 flex flex-col gap-2 items-center text-xs text-text-muted font-mono pt-2">
                <div className="flex items-center justify-between w-full">
                  ...
                  {resendStatus && (
                    <span className={`text-[10px] font-mono ${
                      resendStatus.startsWith('Your code:') ? 'text-cyan font-bold tracking-widest' : 'text-text-muted'
                    }`}>
                      {resendStatus}
                    </span>
                  )}
                ...
                <p className="text-[9px] text-text-muted/60 text-center">
                  Didn't receive it? Click "Resend Code" to get your code.
                </p>
              </div>
          </form>
        </div>
