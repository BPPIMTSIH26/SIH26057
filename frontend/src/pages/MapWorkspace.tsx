...
export default function MapWorkspace() {
  ...
  // Event-driven Radar sync and zoom visibility without 60fps layout thrashing
  useEffect(() => {
    ...
    // Hook onto map movement rather than running an infinite unthrottled loop
    const onMapMove = () => {
      if (animId) cancelAnimationFrame(animId);
      animId = requestAnimationFrame(syncRadar);
    };
  ...
  const initialVp = useMemo(() => getHarbourViewport(harborConfig, null), [harborConfig]);
  ...
  // Jump map smoothly when harbour changes or data loads
  useEffect(() => {
    if (isFirstRender.current) {
      isFirstRender.current = false;
      const t = setTimeout(() => {
        fitMapToHarbourAndPoints(mapRef.current, harborConfig, filteredAnomalies);
      }, 400);
      return () => clearTimeout(t);
    }
    fitMapToHarbourAndPoints(mapRef.current, harborConfig, filteredAnomalies);
  }, [activeHarbour, harborConfig, filteredAnomalies]);

  const graticuleData = useMemo(() => {
    const centerLng = harborConfig?.lng ?? 72.85;
    const centerLat = harborConfig?.lat ?? 18.95;
    return generateGraticule(0.02, formatLat, formatLng, {
      minLat: centerLat - 1.5,
      maxLat: centerLat + 1.5,
      minLng: centerLng - 1.5,
      maxLng: centerLng + 1.5,
    });
  }, [harborConfig, formatLat, formatLng]);
          ...
          {showGraticule && (
            <Source id="graticule" type="geojson" data={graticuleData}>
              <Layer
                ...
                type="line"
                minzoom={9}
                paint={{ 'line-color': '#FFFFFF', 'line-width': 1, 'line-opacity': 0.03 }}
              />
              <Layer
                id="graticule-label"
                type="symbol"
            ...
            return (
              <Marker key={anomaly.id} longitude={anomaly.longitude} latitude={anomaly.latitude} anchor="center">
                <div
                  ...
                  className="relative group cursor-pointer flex flex-col items-center justify-center transition-opacity duration-300 opacity-100 pointer-events-auto"
                  onClick={e => {
                    e.stopPropagation();
